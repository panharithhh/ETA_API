...
class PickupBookRequest(BaseModel):
    ...
    origin_branch_id: Optional[int] = Field(None, description="Branch the sender is nearest to")
    destination_branch_id: Optional[int] = Field(None, description="Branch nearest to the receiver")
...
class DropoffRequest(BaseModel):
    branch_id: int = Field(..., description="ID of the origin branch where package is being checked in")
